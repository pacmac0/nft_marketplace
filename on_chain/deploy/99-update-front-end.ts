...
async function updateAbi() {
	...
	fs.writeFileSync(
		`${frontEndAbiLocation}MockNft.json`,
		mockNft.interface.format(ethers.utils.FormatTypes.json).toString()
	)
	fs.writeFileSync(
		`${frontEndAbiLocation2}MockNft.json`,
		mockNft.interface.format(ethers.utils.FormatTypes.json).toString()
	)
}
...
async function updateContractAddresses() {
	...
	if (chainId in contractAddresses) {
		...
		if (!contractAddresses[chainId]["MockNft"].includes(mockNft.address)) {
			contractAddresses[chainId]["MockNft"].push(mockNft.address)
		}
	} else {
		contractAddresses[chainId] = { NftMarketplace: [nftMarketplace.address] }
		contractAddresses[chainId] = { MockNft: [mockNft.address] }
	}
